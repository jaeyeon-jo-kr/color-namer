(ns color-namer.color-picker
  (:require [color-namer.app :as app]
            [cljs.reader]
            [clojure.string]
            [clojure.async :as a]
...
(defn send-input [input]
  (prn input)
  (http/post "/color/register" {:edn-params input}))
    ...
    (fn []
      [:div {:id "color-picker-component"}
       [:h4 {:style {:padding "20px"
                     :background-color @component-state
                     :font-color "white"}}
        "Color picked : " @component-state]
       [:input
        {:type "color"
         :on-change
         (fn [e]
...
(defn load-colors-info
  [colors-info]
  (a/go (let [response (a/<! (http/get "http://localhost:3000/color/find/all"))]
